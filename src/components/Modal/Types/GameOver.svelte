<script>
	import { DIFFICULTIES, DIFFICULTY_CUSTOM, GAME_OVER_CELEBRATIONS } from '../../../domain/constants.js';
	...
	$: hintsUsedValue = gs?.hintsUsed ?? 0;
	$: difficultyValue = $difficulty ?? 'easy';

	const celebration = GAME_OVER_CELEBRATIONS[Math.floor(Math.random() * GAME_OVER_CELEBRATIONS.length)];
...
<div class="flex flex-col text-center items-center">
	...
	<h1 class="text-4xl font-semibold">{celebration}</h1>
</div>
...
<div class="game-data space-y-2">
	...
	<div class="game-data-item">
		...
		<div class="item-data">
			...
			<div class="item-data-value">
				{difficultyValue === DIFFICULTY_CUSTOM ? 'Custom' : DIFFICULTIES[difficultyValue] || 'Unknown'}
			</div>
		</div>
	</div>
